...
RAYGUIAPI int GuiPanelRounded(Rectangle bounds, int border_width, float roundness, int segments, const char *text);
...
RAYGUIAPI int GuiButtonRounded(Rectangle bounds, int border_width, float roudness, int segments, const char *text);
...
RAYGUIAPI int GuiDropdownBoxRounded(Rectangle bounds, int border_width, float roundness, int segments, const char *text, int *active, bool editMode);          // Dropdown Box control, returns selected item
...
int GuiPanelRounded(Rectangle bounds, int border_width, float roundness, int segments, const char *text)
{
    #if !defined(RAYGUI_PANEL_BORDER_WIDTH)
        #define RAYGUI_PANEL_BORDER_WIDTH   1
    #endif
    ...
    GuiDrawRectangleRounded(bounds, border_width, roundness, segments, GetColor(GuiGetStyle(DEFAULT, (state == STATE_DISABLED)? BORDER_COLOR_DISABLED: LINE_COLOR)),
                     GetColor(GuiGetStyle(DEFAULT, (state == STATE_DISABLED)? BASE_COLOR_DISABLED : BACKGROUND_COLOR)));
    //--------------------------------------------------------------------
...
int GuiButtonRounded(Rectangle bounds, int border_width, float roundness, int segments, const char *text)
{
  int result = 0;
  GuiState state = guiState;
  ...
  // Draw control
  //--------------------------------------------------------------------
  GuiDrawRectangleRounded(bounds, border_width, roundness, segments, GetColor(GuiGetStyle(BUTTON, BORDER + (state*3))), GetColor(GuiGetStyle(BUTTON, BASE + (state*3))));
  GuiDrawText(text, GetTextBounds(BUTTON, bounds), GuiGetStyle(BUTTON, TEXT_ALIGNMENT), GetColor(GuiGetStyle(BUTTON, TEXT + (state*3))));
...
int GuiToggleRounded(Rectangle bounds, int border_width, float roundness, int segments, const char *text, bool *active)
{
  int result = 0;
  GuiState state = guiState;
  ...
  if (state == STATE_NORMAL)
  {
    GuiDrawRectangleRounded(bounds, border_width, roundness, segments, GetColor(GuiGetStyle(TOGGLE, ((*active)? BORDER_COLOR_PRESSED : (BORDER + state*3)))), GetColor(GuiGetStyle(TOGGLE, ((*active)? BASE_COLOR_PRESSED : (BASE + state*3)))));
    GuiDrawText(text, GetTextBounds(TOGGLE, bounds), GuiGetStyle(TOGGLE, TEXT_ALIGNMENT), GetColor(GuiGetStyle(TOGGLE, ((*active)? TEXT_COLOR_PRESSED : (TEXT + state*3)))));
  }
  else
  {
    GuiDrawRectangleRounded(bounds, border_width, roundness, segments, GetColor(GuiGetStyle(TOGGLE, BORDER + state*3)), GetColor(GuiGetStyle(TOGGLE, BASE + state*3)));
    GuiDrawText(text, GetTextBounds(TOGGLE, bounds), GuiGetStyle(TOGGLE, TEXT_ALIGNMENT), GetColor(GuiGetStyle(TOGGLE, TEXT + state*3)));
  }
...
int GuiDropdownBoxRounded(Rectangle bounds, int border_width, float roundness, int segments, const char *text, int *active, bool editMode)
{
    int result = 0;
    GuiState state = guiState;
    ...
    if (editMode)
    {
        ...
        for (int i = 0; i < itemCount; i++)
        {
            ...
            if (i == itemSelected)
            {
		GuiDrawRectangleRounded(itemBounds, border_width, roundness, segments, GetColor(GuiGetStyle(DROPDOWNBOX, BORDER_COLOR_PRESSED)), GetColor(GuiGetStyle(DROPDOWNBOX, BASE_COLOR_PRESSED)));
		GuiDrawText(items[i], GetTextBounds(DROPDOWNBOX, itemBounds), GuiGetStyle(DROPDOWNBOX, TEXT_ALIGNMENT), GetColor(GuiGetStyle(DROPDOWNBOX, TEXT_COLOR_PRESSED)));
            }
            else if (i == itemFocused)
            {
		GuiDrawRectangleRounded(itemBounds, border_width, roundness, segments, GetColor(GuiGetStyle(DROPDOWNBOX, BORDER_COLOR_FOCUSED)), GetColor(GuiGetStyle(DROPDOWNBOX, BASE_COLOR_FOCUSED)));
                GuiDrawText(items[i], GetTextBounds(DROPDOWNBOX, itemBounds), GuiGetStyle(DROPDOWNBOX, TEXT_ALIGNMENT), GetColor(GuiGetStyle(DROPDOWNBOX, TEXT_COLOR_FOCUSED)));
            }
            else GuiDrawText(items[i], GetTextBounds(DROPDOWNBOX, itemBounds), GuiGetStyle(DROPDOWNBOX, TEXT_ALIGNMENT), GetColor(GuiGetStyle(DROPDOWNBOX, TEXT_COLOR_NORMAL)));
        }
...
int GuiSliderRoundedPro(Rectangle bounds, int border_width, float roundness, int segments, const char *textLeft, const char *textRight, float *value, float minValue, float maxValue, int sliderWidth)
{
  ...
  if ((state != STATE_DISABLED) && !guiLocked)
  {
    ...
    if (guiControlExclusiveMode) // Keep dragging outside of bounds
    {
      if (IsMouseButtonDown(MOUSE_LEFT_BUTTON))
      {
	if (CHECK_BOUNDS_ID(bounds, guiControlExclusiveRec))
...
static void GuiDrawRectangleRounded(Rectangle rec, int borderWidth, float roundness, int segments, Color borderColor, Color color)
{
    ...
    {
      // Draw rectangle border lines with color
      DrawRectangleRoundedLinesEx((Rectangle) { (int)rec.x, (int)rec.y, (int)rec.width, (int)rec.height}, roundness, segments, borderWidth, borderColor);
    }
